...
static int k3_ddr_probe(struct platform_device *pdev)
{
	...
	/* Find lpddr4 child node */
	child_np = of_get_child_by_name(dev->of_node, "lpddr4");
	if (!child_np)
		return dev_err_probe(dev, -ENODEV,
				     "lpddr4 child node not found\n");
...
static void k3_ddr_remove(struct platform_device *pdev)
{
	...
	/* Mask all interrupts */
	ret = regmap_field_write(ddr->cfg_reg[K3_DDR_INT_MASK_MASTER], 1U);
	if (ret)
		dev_err(ddr->dev, "Failed to mask interrupt!\n");

	/* Cancel any pending work */
	cancel_work_sync(&ddr->intr_work);
}
