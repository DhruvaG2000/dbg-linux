						    ...
						    bool s2idle)
{
	...
	if (s2idle) {
		printk("\nS2IDLE_DBG: cpu_idle_enter: cpu = %d, state = 0x%x, ret = %d, need_resched = %d\n",
			dev->cpu, state, ret, need_resched());
	}
	trace_psci_domain_idle_enter(dev->cpu, state, s2idle);
	ret = psci_cpu_suspend_enter(state) ? -1 : idx;
	trace_psci_domain_idle_exit(dev->cpu, state, s2idle);
	if (s2idle) {
		printk("\nS2IDLE_DBG: cpu_idle_exit: cpu = %d, state = 0x%x, ret = %d, need_resched = %d\n",
			dev->cpu, state, ret, need_resched());
	}
